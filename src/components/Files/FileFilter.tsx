...
export function FileFilter({ viewMode, setViewMode }: FileFilterProps) {
  ...
  return (
    <div className="flex items-center justify-end gap-3 mt-4 px-4">
      ...
      <Button
        ...
        className="bg-orange-400 hover:bg-orange-400 cursor-pointer flex items-center gap-2 p-1 rounded-[3px] px-2 h-6"
      >
        <div className="w-3 h-3 bg-white rounded-full flex items-center justify-center">
          <Plus className="text-orange-400 !w-2 !h-2" strokeWidth={3} />
        </div>
        <p className="text-xs 3xl:!text-sm text-white font-light">New Folder</p>
      </Button>
      <NewFolderDialog open={openNewFolder} onOpenChange={setOpenNewFolder} />
      <Button
        onClick={() => navigate({ to: "/files/new-file" })}
        className="bg-[#2F80ED] hover:bg-[#2F80ED] flex items-center gap-2 p-1 rounded-[3px] px-2 h-6 cursor-pointer"
      >
        <div className="w-3 h-3 bg-white rounded-full flex items-center justify-center">
          <Plus className="text-[#2F80ED] !w-2 !h-2" strokeWidth={3} />
        </div>
        <p className="text-xs 3xl:!text-sm text-white font-light">New File</p>
      </Button>
    </div>
  );
}
